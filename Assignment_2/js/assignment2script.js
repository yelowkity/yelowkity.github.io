...
function submitNewGroup() {
  ...
  fetchPromise
    ...
    .catch((err) => {
      console.log(err);
      document.getElementById("postNewGroupContent").innerHTML = "Invalid person : " + data.firstName;
    });
...
function searchTask() {
  ...
  let task = document.getElementById("typeAhead").value;
  //console.log(task);
  ...
  fetch('./alltasks', settings)
    ...
    .then((json) => {
      ...
      json.data.forEach(element => {
        console.log(element);
        if (element.taskName.match(regex)) {
          groupValues.push(element.taskName);
        }
      });   
     ...
     const result = groupValues.map(res => {

        //const regex2 = new RegExp(this.value, 'gi');
        const res2 = res.replace(regex, '<span style="color:red;">'+task+'</span>');

        return '<li><span>'+res2+'</span></li>';
